<script>
  ...
  let exportTimetable = () => {
    ...
    console.log(arr2d);
    let fileName = deptSelected + "_rooms.csv";
    if (allRoomFlag) fileName = "all_rooms.csv";
  ...
  /* react to change */
  $: getLessons(deptSelected, weekSelected, allRoomFlag);
...
<div class="container-fluid  no-print">
    ...
    {#if !allRoomFlag}<button on:click={()=>allRoomFlag=!allRoomFlag} class="btn" type="button"><i class="fas fa-align-justify"></i></button>{/if}
			{#if allRoomFlag}<button class="btn btn-primary" on:click={()=>allRoomFlag=!allRoomFlag} type="button"><i class="fas fa-align-justify"></i></button>{/if}
